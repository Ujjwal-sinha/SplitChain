...
export async function initializeDatabase() {
  try {
    ...
    await db.collection('balances').createIndex({ group_id: 1, user_address: 1 }, { unique: true })
...
export async function createGroup(name: string, creator: string, members: string[], txHash?: string) {
  const { db } = await connectToDatabase()
...
export async function addExpense(groupId: string, payer: string, amount: string, token: string, description: string, txHash?: string) {
  const { db } = await connectToDatabase()
  const { ObjectId } = require('mongodb')
